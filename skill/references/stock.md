...
Use this reference only after the scope gate admits the stock market. Only a canonical Wind code or previously validated identity goes directly to its property tool; unresolved identity or conditional screening uses the search row first.
...
| `get_stock_basicinfo` | Current identity, listing board/date, industry, concepts, business profile, registration facts, or current status; validate a discovered candidate's company, code, and market before any property tool. | Current/static profile, not status history, financials, or holders. |
...
| `search_stocks` | Resolve a company name or non-canonical code, or filter supported stocks by market, industry, concept, price, valuation, financial, fund-flow, or technical conditions. | Do not guess an exchange suffix. Skip only for a validated canonical identity; output is an entity list, not computed analytics. |
