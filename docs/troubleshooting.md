...
| `WIND_BALANCE` / `WIND_AUTH` | Exact structured balance or authentication evidence was classified, or the upstream answered HTTP 401/403 (Wind's edge returns an HTML page, not a structured envelope, for a rejected Key). | The affected slot is disabled and the cursor advances. Correct funding or credentials, replace the binding if needed, then follow the restore runbook. If every slot lands in `disabled_auth` within one invocation, suspect a Wind-side authentication outage rather than five bad Keys: validate one Key with `credential:validate`, then restore. |
