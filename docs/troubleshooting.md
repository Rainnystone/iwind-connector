...
| `KEY_POOL_EXHAUSTED` | This invocation has tried every eligible slot once, including an unclassified walk, or no slot is currently eligible. | Inspect `currentSlotId`, slot states, and trusted reset times. A later independent request may re-probe slots that stayed active, including unknown and daily-quota slots that had no trusted reset; balance/auth/manual states require correction and restore. Never guess a reset time. |
| `WIND_DAILY_QUOTA` | Exact structured daily-quota evidence was classified, or the upstream answered HTTP 401/403 with no structured vendor code. | The primary cursor advances through `key-05 → key-04 → key-03 → key-02 → key-01 → key-05`. A trusted future reset keeps the affected slot unavailable until then. A status-only 401/403 has no trusted reset, so the slot stays active and a later wrap may probe it. Confirm the notice and inspect status. |
| `WIND_BALANCE` / `WIND_AUTH` | Exact structured balance or authentication evidence was classified. HTTP 401/403 alone is daily quota, not auth. | The affected slot is disabled and the cursor advances. Correct funding or credentials, replace the binding if needed, then follow the restore runbook. |
...
| `WIND_UNKNOWN` / `WIND_REQUEST_FAILED` | The input did not match a trusted classification, or the request stopped safely. | An unclassified Wind failure tries each remaining active key once and leaves those slots active. Correlate by request ID, allowlisted HTTP status, and bounded vendor code. Do not treat a missing code as daily quota. |
