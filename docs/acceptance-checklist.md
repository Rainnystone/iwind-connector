...
- [ ] For an existing Worker, use the complete owner-only Secret file to `versions upload`, inspect the candidate by binding names only, then deploy the exact `candidate@100%`; do not percentage-split the KeyPool deployment.
- [ ] For first creation only, use one complete `wrangler deploy --secrets-file` deployment; do not use per-binding `secret put` as a standard deployment step.
