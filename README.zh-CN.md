...
| 按额度自动轮换 | 当前发行版默认提供两个有序槽位；底层按优先级选槽的模型可以扩展到更多 Key，不需要改变 MCP 或 Skill 接口。 |
...
当前发行版提供两个有顺序的槽位：`key-01` 和 `key-02`。

1. `key-01` 可用时，所有请求都优先使用它。
2. 只有明确归类为额度、余额、认证或人工状态允许切换时，网关才会改变槽位。
3. 日额度耗尽可以让后续请求转到 `key-02`。
4. QPS 限流、并发错误、超时、网络故障、上游 5xx 和未知错误不会连续烧掉 Key 池。
5. 发生轮换、轮换失败或整个池不可用时，Agent 会收到脱敏提示，但不会看到 Key 或底层基础设施细节。

这是按顺序使用、额度耗尽后切换的 failover，不是 round-robin 负载均衡，也不能用于规避万得的账户、合同或服务限制。只能把你依法有权共同使用的 Key 放进同一个池。

KeyPool 内核会保存各个槽位的明确优先级，并选择优先级最高且当前可用的槽位，因此未来可以用同一套严格串行规则继续扩展 `key-03`、`key-04` 等更多槽位。不过，当前发行版不会自动发现新增的 Secret：增加第三枚 Key 仍需同步更新槽位契约、Secret binding 映射、部署配置、管理接口、测试和文档，然后重新部署网关。仅仅扩大 KeyPool 不需要改变 MCP 地址、31 工具清单、OAuth 流程或 Skill 包。
...
- 两枚万得 API Key，对应当前仓库内置的两个槽位；绝对不能把它们提交进仓库。
...
- 添加你准备采用的开源许可证。这个仓库目前没有 `LICENSE` 文件，因此现在还不能宣称已经按某个许可证开源。
- 使用你自己的私有 Key 文件运行完整测试、确定性打包和精确 Secret 扫描。
- 确认 Git 历史中没有私有部署记录、账户标识、OAuth artifact、带临时参数的 callback URL、业务响应或凭据。
- 在重新分发生成的契约信息或向其他人提供服务前，检查你与万得之间的协议。
- 除非你重新设计、批准、实现并审查新的安全边界，否则始终保持只读。

Wind 和 iWind 相关产品名称属于其权利人。本仓库是独立的集成项目，不提供、转售或授予任何万得数据访问权限。
